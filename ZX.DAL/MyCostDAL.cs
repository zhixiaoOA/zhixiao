...
namespace ZX.DAL
{
    public class MyCostDAL : DBBase<MyCost>
    {
        #region 分页获取数据列表
        /// <summary>
        /// 分页获取数据列表
        /// </summary>
        /// <param name="key">用途</param>
        ...
        /// <returns></returns>
        public DataList<MyCostModel> GetMyCostList(string key, long userId, long appUserId, string beginTime, string endTime, string status, int pageIndex, int pageSize)
        {
            string sql = "Proc_GetMyCostList";
            Pmts.ClearPmts();
            Pmts.Add("key", key);

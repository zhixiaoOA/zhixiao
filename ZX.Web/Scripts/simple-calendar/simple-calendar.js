...
var SimpleCalendar = function () {
    ...
    _createClass(SimpleCalendar, [{
        ...
        value: function create() {
            ...
            for (var i = 0; i < 35; i++) {//ondblclick="onCkDay(1,this)"    ondblclick="onCkDay(3,this)"
                days.innerHTML = days.innerHTML + '<div class="sc-item" ondblclick="onCkDay(2,this)"><div class="day" ></div><div class="lunar-day" ></div></div>';
            }
            //添加下拉框数据
        ...
        value: function addData(year, month) {
            ...
            for (var i = 0; i < 35; i++) {
                ...
                daysElement[i].querySelector('.day').setAttribute("year", writeyear);
                daysElement[i].querySelector('.day').setAttribute("month", writemonth);
                //判断是否添加阴历
        ...
        value: function updateMark(year, month) {
            ...
            if (options.showMark) {
                ...
                //取得节日数据
                data = options.markIds;
                if (data) {
                    daysElement.forEach(function (v, i) {
                        var day = +v.querySelector('.day').innerHTML;
                        if (day == 1) currentmonth++;
                        if (data[year + '-' + currentmonth + '-' + day]) {
                            v.setAttribute("vid", data[year + '-' + currentmonth + '-' + day]);
                        }
                    });
                }
            }
        ...
        key: 'addMark',
        value: function addMark(day, info, id) {
            this._options.mark[day] = info;
            this._options.markIds[day] = id;
            this.update();
        }
    }, {
        key: 'cancelMark',

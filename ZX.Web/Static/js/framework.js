...
function tabiframeId() {
    var tabs_container = top.$("#pageContent");
    return "tabs_iframe_" + tabs_container.attr('id').substr(12);
}
//关闭当前tab
function back() {
    top.ThisCloseTab();
...
请求Ajax 带返回值
*/
function getTextAjax(url, postData, callBack) {
    $.ajax({
        type: 'post',
        dataType: "text",
        url: RootPath() + url,
...
function GetFormData(element) {
    $(element).find('input[type="checkbox"]').each(function () {
        var target = $(this);
        $('input[name="' + target.attr('id') + '"]').val(target.prop('checked'));
    });
...
弹出对话框（没按钮）
*/
function Dialog(url, _id, _title, _width, _height,closeFn) {
    Loading(true);
    top.$.dialog({
        id: _id,
        width: _width,
...
function IsChecked(id) {
    var isOK = true;
    if (id == undefined || id == "0" || id == "" || id == 'null' || id == 'undefined') {
        isOK = false;
        tipDialog('您没有选中任何项,请您选中后再操作。', 4, 'warning');
...
function bindRightMenu(element) {
    var oMenu = $('.rightMenu');
...
jQuery.fn.customInput = function () {
    return $('input').each(function () {
        if ($(this).is('[type=checkbox],[type=radio]')) {
            ...
            if (input.attr('id') == undefined) return true;
            // 使用输入的ID得到相关的标签
			...
			.click(function () {
			    ...
			    $('input[data-name="' + $(this).attr('data-name') + '"]').trigger('updateState');
			})
			.focus(function () {
...
function autocomplete(Objkey, width, height, data, callBack) {
    ...
    $("#div_gridshow").find('tbody tr').click(function () {
        ...
        $(this).find('td').each(function (i) {
            parameter += '"' + $(this).attr('data-id') + '"' + ':' + '"' + $.trim($(this).text()) + '",'
        });
        if ($('#' + Objkey).attr('readonly') == 'readonly') {
            return false;
        }
...
/*count:      数量
--------------------------------------------------*/
function blackListConfig(url, parm, count) {
    if (count == undefined) {
        count = 1;
    }
    confirmDialog("系统提示", "注：您确定要将 " + count + " 笔记录加入黑名单吗？", function (r) {
        if (r) {
            Loading(true, "正在提交数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
...
/*count:      数量
--------------------------------------------------*/
function verifyConfig(url, parm, count) {
    if (count == undefined) {
        count = 1;
    }
    confirmDialog("系统提示", "注：共 " + count + " 笔记录,是否确定审核？", function (r) {
        if (r) {
            Loading(true, "正在提交数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
...
function BindDropItem(ControlId, Code, Memo) {
    ...
    getAjax('/Common/GetDictionaryDetailByDictionaryCode', { dictionaryCode: Code }, function (result) {
        if (!result.isTree) {
            var options = [];
            $.each(result.data, function (i, item) {
                options.push('<option value="' + item.value + '">' + item.text + '</<option>');
            });
            $(ControlId).append(options.join(''));
        }
        Loading(false);
    });
}
...
//加载权限按钮
function PartialButton(moduleId) {
    var JsonData = [], toolbar_html = "", toolbar_Children_Count = 0;
    getAjax("/Admin/Common/LoadButton", { moduleId: moduleId }, function (Data) {
        JsonData = Data;
        $.each(Data, function (i, button) {
            if (button.ParentId == 0 && button.ButtonCategory == 10) {
                var toolbar_Children_List = Toolbar_Children(button.Id, Data)
                if (toolbar_Children_Count > 0) {
                    toolbar_html += "<a id=\"" + button.ButtonCode + "\" class=\"tools_btn dropdown\">";
                    toolbar_html += "<div style=\"float: left;\"><div class=\"icon\"><img src=\"/Areas/Admin/Content/images/Icon16/" + button.Icon + "\" /></div><div class=\"text\">" + button.ButtonName + "</div></div>";
                    toolbar_html += "<div class=\"dropdown-icon\"><img src=\"/Areas/Admin/Content/images/dropdown-icon.png\" /></div>";
                    ...
                    var title = button.ButtonTitle == null ? button.ButtonName : button.ButtonTitle;
                    toolbar_html += "<a id=\"" + button.ButtonCode + "\" title=\"" + title + "\" onclick=\"" + button.JSEvent + "\" class=\"tools_btn\"><span><b style=\"background: url('/Areas/Admin/Content/images/Icon16/" + button.Icon + "') 50% 4px no-repeat;\">" + button.ButtonName + "</b></span></a>";
                }
                if (button.IsSplit) {
    ...
    $('.tool_bar .PartialButton').html(toolbar_html);
    function Toolbar_Children(ButtonId, Data) {
        var _toolbar_Children = "";
        toolbar_Children_Count = 0;
        $.each(Data, function (i, button) {
            if (button.ParentId == ButtonId) {
                var title = button.ButtonTitle == null ? button.ButtonName : button.ButtonTitle;
                _toolbar_Children += "<li id=\"" + button.ButtonCode + "\" title=\"" + title + "\" onclick=\"" + button.JSEvent + "\"><img src=\"/Areas/Admin/Content/images/Icon16/" + button.Icon + "\" />" + button.ButtonName + "</li>";
                if (button.IsSplit) {
                    _toolbar_Children += "<div class=\"m-split\"></div>";
                }
    ...
    $(".tool_bar .dropdown").hover(function () {
    ...
    //右击菜单
    var right_toolbar_Count = 0, right_toolbar_html = "";
    right_toolbar_html += '<div class="rightMenu"><ul>';
    $.each(JsonData, function (i, button) {
        if (button.ParentId == '0' && button.ButtonCategory == 20) {
            var title = button.ButtonTitle == null ? button.ButtonName : button.ButtonTitle;
            var righttoolbar_Children_List = right_toolbar_Children(button.Id, JsonData);
            if (right_toolbar_Count > 0) {
                right_toolbar_html += "<li id=\"right_" + button.ButtonCode + "\" title=\"" + title + "\" onclick=\"" + button.JSEvent + "\" ><img src=\"/Areas/Admin/Content/images/Icon16/" + button.Icon + "\" />" + button.ButtonName + righttoolbar_Children_List + "</li>";
            } else {
                right_toolbar_html += "<li id=\"right_" + button.ButtonCode + "\" title=\"" + title + "\" onclick=\"" + button.JSEvent + "\" ><img src=\"/Areas/Admin/Content/images/Icon16/" + button.Icon + "\" />" + button.ButtonName + "</li>";
            }
            if (button.IsSplit) {
    ...
    function right_toolbar_Children(ButtonId, JsonData) {
        ...
        $.each(JsonData, function (i, button) {
            if (button.ParentId == ButtonId) {
                var title = button.ButtonTitle == null ? button.ButtonName : button.ButtonTitle;
                _right_toolbar += "<li id=\"right_" + button.ButtonCode + "\" title=\"" + title + "\" onclick=\"" + button.JSEvent + "\" ><img src=\"/Areas/Admin/Content/images/Icon16/" + button.Icon + "\" />" + button.ButtonName + "</li>";
                if (button.IsSplit) {
...
function PartialButtonNew(moduleId)
{
    var url = '/Admin/Base_ButtonPermission/GetUserButton'
    $.post(url, { 'moduleid': moduleId }, function (data) {
        var buttonHtml = '';
        $.each(data, function (index, item) {
            //buttonHtml += '' + item.ButtonHtml + '';
            buttonHtml += '' + item.ButtonTitle + '';
            
        });
        $("#pageButtonList").html(buttonHtml);
    });
}
//=================动态菜单tab标签========================
...
//    if (value) return '<img src="/Areas/Admin/Content/images/checkokmark.gif" alt="" />';
//    else return '<img src="/Areas/Admin/Content/images/checknomark.gif" alt="" />';
//}

function layerDialog(_url, _title, _callBack) {
    ...
    parent.layer.open({
        ...
        yes: function (index, layero) {
            _callBack('layui-layer-iframe' + index);
            return false;
...
function closeLayer() {
    //当你在iframe页面关闭自身时
    var index = 1; //parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
    layer.close(index); //再执行关闭
}



function builderUser(data) {
    if (data != null) {
        var html = [];
        $.each(data, function (i, user) {
            var icon = user.Sex == 1 ? 'user_green.png' : 'user_female.png';
            if (user.IsChecked) {
                html.push('<li class="dp_' + user.StoreId + ' selected"><a class="a_selected" data-userid="' + user.Id + '" title="工号：' + user.No + '"><img src="/Areas/Admin/Content/images/Icon16/' + icon + '">' + user.Name + '</a><i></i></li>');
            } else {
                html.push('<li class="dp_' + user.StoreId + '"><a data-userid="' + user.Id + '" title="工号：' + user.No + '"><img src="/Areas/Admin/Content/images/Icon16/' + icon + '">' + user.Name + '</a><i></i></li>');
            }
        });
        return html.join('');
    }
...
function initMinUEditor(id, width, height) {
    UE.getEditor(id, {
        initialFrameWidth: width,
        initialFrameHeight: height,
        serverUrl: '/Static/js/ueditor/net/controller.ashx',
        toolbars: [[
    'fullscreen', 'source', '|', 'undo', 'redo', '|',
    'bold', 'italic', 'underline', 'fontborder', 'strikethrough', 'superscript', 'subscript', 'removeformat', 'formatmatch', 'autotypeset', 'blockquote', 'pasteplain', '|', 'forecolor', 'backcolor', 'insertorderedlist', 'insertunorderedlist', 'selectall', 'cleardoc', '|',
    'rowspacingtop', 'rowspacingbottom', 'lineheight', '|',
    'customstyle', 'paragraph', 'fontfamily', 'fontsize', '|', 'touppercase', 'tolowercase', '|',
    'link', 'unlink', 'anchor', '|', 'imagenone', 'imageleft', 'imageright', 'imagecenter', '|', 'map', 'gmap', 'insertcode', 'webapp', 'pagebreak', 'template', 'background', '|',
    'horizontal', 'date', 'time', 'spechars', 'snapscreen', 'wordimage', '|', 'preview', 'searchreplace', 'help', 'drafts'
        ]]
    });
}

function initUEditor(id, width, height) {
...
function initUploadify(id, callback, isOnlyImg) {
    if (isOnlyImg) {
        $('#' + id).uploadify({
            uploader: '/Admin/File/FileUpload',
            swf: '/Areas/Admin/Content/javascript/uploadify/uploadify.swf',
            buttonText: "上传图片",
            height: 24,
            width: 70,
...
function dgRefresh(id) {
    window.frames[id].refresh();
}
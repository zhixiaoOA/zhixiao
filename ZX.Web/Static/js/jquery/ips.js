...
+function ($, window, document, Math) {
    ...
    entry.prototype.init = function (options, ignoreDefault) {
        ...
        try {
            if (this.icon != null && this.icon.indexOf('icon-') == 0) this.iconhtml = '<i class="icon ' + this.icon + '"></i>';
            else if (this.icon != null && this.icon.length > 0) this.iconhtml = '<img src="' + this.icon + '" alt="" />';
            else {
    ...
    function shortcuts() {
        ...
        this.render = function () {
            ...
            if (this.firstRender) {
                ...
                this.$appsMenu.sortable({
                    trigger: '.app-btn', selector: 'li', start: function () {
...
$(function () {
    $(document).on('click', '#s-menu-94 button,#s-task-94 button', function () {
        if ($("#iframe-94")[0] && $("#iframe-94")[0].contentWindow.location.href.indexOf("PersonalSpace/index") < 0) {
            $("#iframe-94").attr("src", "/PersonalSpace/index?mid=94");
        }
    });
});

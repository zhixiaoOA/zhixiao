...
; var swfobject = function () {
    ...
    M = function () {
        ...
        ae = Y ? /win/.test(Y) : /win/.test(ah), ac = Y ? /mac/.test(Y) : /mac/.test(ah), af = /webkit/.test(ah) ? parseFloat(ah.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : false, X = ! +"\v1", ag = [0, 0, 0], ab = null;
    ...
    function z(aa) {
        var X = null;
        var Y = c(aa); if (Y && Y.nodeName == "OBJECT") {
            if (typeof Y.SetVariable != D) {
                X = Y;
            } else { var Z = Y.getElementsByTagName(r)[0]; if (Z) { X = Z; } }
        } return X;
    }
    ...
    function P(aa, ab, X, Z) {
        a = true; E = Z || null; B = { success: false, id: X };
        var ae = c(X); if (ae) {
            if (ae.nodeName == "OBJECT") {
                l = g(ae); Q = null;
            } else { l = ae; Q = X; } aa.id = R;
    ...
    function v(ac, Y, ad, ab) {
        if (M.ie && M.mac) { return; } var aa = j.getElementsByTagName("head")[0]; if (!aa) {
            return;
        }
        ...
        if (ab) {
            ...
            var Z = C("style"); Z.setAttribute("type", "text/css"); Z.setAttribute("media", X); n = aa.appendChild(Z);
    ...
    return {
        ...
        embedSWF: function (ab, ah, ae, ag, Y, aa, Z, ad, af, ac) {
            ...
            if (M.w3 && !(M.wk && M.wk < 312) && ab && ah && ae && ag && Y) {
                w(ah, false); K(function () {
                    ...
                    aj.width = ae; aj.height = ag; var am = {};
                    if (ad && typeof ad === r) {
        ...
        getFlashPlayerVersion: function () {
            return { major: M.pv[0],
                minor: M.pv[1], release: M.pv[2]
            };
        },
        hasFlashPlayerVersion: F,
        createSWF: function (Z, Y, X) {
            if (M.w3) { return u(Z, Y, X); } else { return undefined; }
        },
        ...
        addDomLoadEvent: K, addLoadEvent: s,
...
SWFUpload.QUEUE_ERROR = {
    ...
    FILE_EXCEEDS_SIZE_LIMIT: -110, ZERO_BYTE_FILE: -120, INVALID_FILETYPE: -130
};
...
SWFUpload.FILE_STATUS = {
    ...
    ERROR: -3, COMPLETE: -4, CANCELLED: -5
};
...
SWFUpload.prototype.destroy = function () {
    try {
        this.cancelUpload(null, false); var a = null; a = this.getMovieElement(); if (a && typeof (a.CallFunction) === "unknown") {
            for (var c in a) {
                try {
                    if (typeof (a[c]) === "function") { a[c] = null }
                } catch (e) { }
        ...
        this.customSettings = null; this.eventQueue = null;
...
SWFUpload.prototype.addSetting = function (b, c, a) {
    if (c == undefined) { return (this.settings[b] = a) } else { return (this.settings[b] = c) }
};
...
SWFUpload.prototype.queueEvent = function (b, c) {
    if (c == undefined) { c = [] } else { if (!(c instanceof Array)) { c = [c] } } var a = this; if (typeof this.settings[b] === "function") {
    ...
    var e = /[$]([0-9a-f]{4})/i; var f = {}; var d; if (c != undefined) {
        for (var a in c.post) {
            if (c.post.hasOwnProperty(a)) {
                d = a; var b;
                while ((b = e.exec(d)) !== null) { d = d.replace(b[0], String.fromCharCode(parseInt("0x" + b[1], 16))) } f[d] = c.post[a]
            }
        } c.post = f
    }
    return c
...
SWFUpload.Console.writeLine = function (d) {
    ...
    try {
        ...
        if (!b) {
            ...
            b.wrap = "off"; b.style.overflow = "auto";
            b.style.width = "700px";
...
(function ($) {
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = '有些文件没有添加到队列:';//Some files were not added to the queue
        ...
        onSelect: function (file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    var replaceQueueItem = confirm('文件:"' + file.name + '"已存在队列中,是否要替换?');
                    if (!replaceQueueItem) {
            ...
            if (settings.itemTemplate == false) {
                settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item" width="100%">\
					<div class="cancel">\
                ...
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            this.queueData.errorMsg += '\n选定的文件超过剩余上传限制 (' + errorMsg + ').';
                        } else {
                            this.queueData.errorMsg += '\n文件已超过队列大小限制 (' + settings.queueSizeLimit + ').';
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + file.name + '" 超过最大上传限制 (' + settings.fileSizeLimit + ').';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n文件 "' + file.name + '" 为空.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + file.name + '" 不属于可上传文件类型 (' + settings.fileTypeDesc + ').';
                        break;
                }
            }
        ...
        onUploadError: function (file, errorCode, errorMsg) {
            ...
            switch (errorCode) {
                ...
                case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
                    errorString = '找不到上传路径';//Missing Upload URL
                    break;
                ...
                case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
                    errorString = '错误';//Validation Error
                    break;
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == 1) {
                            var overwrite = confirm('服务器已经存在一个名为"' + file.name + '"的文件,是否替换现有文件?');
                            if (!overwrite) {
                                this.cancelUpload(file.id);
                                $('#' + file.id).remove();
